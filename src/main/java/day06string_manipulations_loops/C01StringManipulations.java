...
public class C01StringManipulations {
    ...
    public static void main(String[] args) {
        ...
        String last = ccNum.substring(16);
       ...
       int numOfDiffFromSpace = sentence.replace(" ", "").length();

        System.out.println(numOfDiffFromSpace);//22
        /*

        1)concat() method  is for joining two String like "+" sign
        2)replace(char, char) or replace(String, String): It is used to replace single character or multiple characters
            note: If u need to use Regular Expressions use replaceAll() otherwise use replace()
